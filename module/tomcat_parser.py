...
import datetime
import urllib,json
from common.convert import urldecode,htmlunescape
try:
...
class TomcatParser(object):
    ...
    def parse(self, line):
        ...
        # 获取临时的 CDN IP 和 访问文件
        self.real_ip = line_item[0] # 请求IP
        self.access_time = line_item[3].replace("[","") # 请求时间
        self.method = line_item[5].strip('"') # 请求方式
        self.request_url = line_item[6] # 请求的URL
        self.response_status = line_item[8] # NG 响应状态码
        self.bbytes = line_item[9] # NG 响应状态码
